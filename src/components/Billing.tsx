...
export default function Billing({ onPurchase }: { onPurchase: (tierId: string) => void }) {
  return (
    <div className="py-12 space-y-12">
      <div className="text-center max-w-2xl mx-auto space-y-4">
        <h1 className="text-4xl font-bold tracking-tight text-white">Select your Bull Terminal level.</h1>
          ...
          >
            ...
            <div>
              <div className="flex items-center gap-3 mb-4">
                <div className={`p-2 rounded-lg ${tier.popular ? 'bg-green-500/20 text-green-400' : 'bg-slate-500/20 text-slate-400'}`}>
              ...
              <ul className="space-y-3 mb-8">
                {tier.features.map((feature, i) => (
                  <li key={i} className="flex items-center gap-3 text-xs opacity-80 text-slate-300">
                    <Check className={`w-4 h-4 shrink-0 ${tier.popular ? 'text-green-500' : 'text-blue-500'}`} />
                    {feature}
                  </li>
                ))}
              </ul>
